...
class EventController @Autowired constructor(val eventService: EventService) {
    ...
    fun getEvent(@PathVariable(required = true) realmId: String,
                 @PathVariable uid: String, request: HttpServletRequest): Event? {

        // HATEOAS
//        val currentRealm = request.getAttribute(RealmLoaderFilter.REALM_ATTRIB) as Realm?
//        var representation = EntityModel<Event>(eventService.find(uid, true))
//        representation.add(linkTo(methodOn(
//                PostController::class.java).getPosts(currentRealm.uid!!, currentRealm.forumUid!!, null)
//        ).withRel("post"))
//        return representation

        val retval = eventService.find(uid, true)
